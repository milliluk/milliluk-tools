...
HELP_OUTPUT = b"""usage: max2png [-h] [--color] [--swap] infile outfile""" \
...
b"""     swap blue/orange\n\nCopyright 2016 Erik Gavriluk. Released under """ \
...
def generic_test_output(infile, expected_outfile, args):
    ...
    expected_outfile = os.path.join(os.path.dirname(__file__), 'samples',
                                    expected_outfile)
    outfile = NamedTemporaryFile()
    outfile.close()
    assert check_output('max2png {} {} {}'.format(args, infile, outfile.name),
                        shell=True) == b''
