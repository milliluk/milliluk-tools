...
HELP_OUTPUT = b"""usage: cgp220 [-h] [-c] [-m] [-y] [-k] [-d D] infile """ \
...
b"""Gavriluk. Released under the Artistic License 2.0.\n"""
...
def test_output():
    ...
    expected_outfile = os.path.join(os.path.dirname(__file__), 'data', 'cgp220.prn.png')
    outfile = NamedTemporaryFile()
    outfile.close()
    assert check_output('cgp220 {} {}'.format(infile, outfile.name), shell=True) == b''
